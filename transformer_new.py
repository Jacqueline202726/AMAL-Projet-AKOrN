...
class SudokuTransformer(nn.Module):
    ...
    def forward(self, x, is_input):
        ...
        x = self.out(x)
        #new part in here 
        #use the is_input mask to mask out the values that are not input
        x=x.view(-1, 9, 9, 9)
        # print('x.size()',x.size())
        # print('originx.size()',originx.size())
        # print('is_input.size()',is_input.size())
        x_out = originx * is_input + x * (1 - is_input) #[b,h,w,9]+
        return x_out
